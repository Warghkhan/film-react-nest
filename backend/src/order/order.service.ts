...
import { Schedule } from '../films/schemas/schedule.schema';

@Injectable()
export class OrderService {
  constructor(@InjectModel(Film.name) private filmModel: Model<Film>) {}

  async createOrder(tickets: OrderItemDto[]): Promise<{ total: number; items: OrderResponseDto[] }> {
  ...
  for (const ticket of tickets) {  
    ...
    if (!schedule) throw new NotFoundException(`Session ${session} not found`);
